...
- **鉴权**：演示级双角色令牌（HMAC-SHA256 签名 + 7 天时效覆盖评审周期 + 恒时比较防时序攻击），密钥仅存服务端环境变量
...
1. **双入口登录门禁**：首屏「用户入口 / 技术人员入口」分流。用户入口免密（昵称仅前端留存）；技术入口校验 `ADMIN_PASSWORD`（恒时比较），签发 HMAC-SHA256 时效令牌（7 天有效，覆盖评审周期），后续请求经 `Authorization: Bearer` 鉴权，篡改/过期令牌一律拒绝。评审演示口令直接标注在技术入口输入框下方，非技术工作人员照输即可体验。
