...
const { buildSystemPrompt, detectRole, entryHint } = require('./prompt');
...
async function handleChat(input, authInfo) {
  ...
  // 动态 System Prompt 注入：入口身份基调（登录角色，会话级）+ 本条消息关键词识别（学段/身份，消息级）
  const entry = authInfo && authInfo.role !== 'admin' ? entryHint(authInfo.role) : null;
  const roleHint = detectRole(msg + ' ' + history.filter((m) => m.role === 'user').map((m) => m.content).join(' '));
  const messages = [
    { role: 'system', content: buildSystemPrompt(chunks, roleHint, entry) },
    ...history,
  ...
  try {
    ...
    if (authInfo && authInfo.role === 'admin') {
      data.trace = { ...trace, roleHint, entryRole: authInfo.role, totalMs: Date.now() - t0, model: process.env.LLM_MODEL || 'deepseek-chat' };
    }
    return ok(data);
  } catch (e) {
...
function handleLogin(input) {
  ...
  return ok({ token: r.token, role: r.role, expiresIn: 7 * 24 * 3600 });
}
