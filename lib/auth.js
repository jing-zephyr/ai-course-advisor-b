...
// 演示级鉴权：四入口角色——user 用户（学生/家长）/ teacher 教师 / biz 企业机构 / admin 技术人员管理
// 前三个为免密自选身份（决定界面主题与提示词语气），admin 为口令登录（开启观测面板）
// 令牌为 HMAC-SHA256 签名的时间戳凭证，7 天过期（覆盖评审周期，评审人员一次登录即可）；密钥仅存服务端环境变量，不落仓库
const TTL_MS = 7 * 24 * 3600 * 1000;
const ROLES = ['user', 'teacher', 'biz', 'admin'];
...
function verify(token) {
  ...
  const m = token.match(/^aca\.(user|teacher|biz|admin)\.(\d+)\.([0-9a-f]{32})$/);
  if (!m) return null;
...
// 登录：用户/教师/企业三入口免密（演示场景，身份仅用于界面主题与提示词差异化）；技术入口校验 ADMIN_PASSWORD
function login(role, password) {
  if (role === 'user' || role === 'teacher' || role === 'biz') return { ok: true, token: issue(role), role };
  if (role === 'admin') {
