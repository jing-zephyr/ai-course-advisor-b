...
function retrieve(query, topK = 5) {
  ...
  // trace：观测面板用——命中块得分、检索词、区池大小（不含正文，避免泄露无关素材全文）
  const top = items.slice(0, topK);
  return {
    route: r,
    chunks: top.map((x) => x.b),
    trace: {
      zone: r,
      terms,
      poolSize: pool.length,
      hits: top.map((x) => ({ id: x.b.id, zone: x.b.zone, doc: x.b.doc, chapter: x.b.chapter, title: x.b.title, score: Math.round(x.s * 10) / 10 })),
    },
  };
}
